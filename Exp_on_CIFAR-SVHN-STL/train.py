...
from networks.config import MANPP_PAPER_LOCAL_MODULES, supported_local_module_nums


ARCH_ALIASES = {
    'resnetInfoPro_MANPP': 'resnetInfoPro_MANPP',
    'resnetDGL_MANPP': 'resnetDGL_MANPP',
    'resnetInfoPro_MAN': 'resnetInfoPro_MANPP',
    'resnetDGL_MAN': 'resnetDGL_MANPP',
}
SUPPORTED_RESNET_LAYERS = (32, 110)


parser = argparse.ArgumentParser(description='MAN++ InfoPro training on CIFAR/SVHN/STL')
parser.add_argument('--dataset', default='cifar10', type=str, choices=['cifar10', 'stl10', 'svhn'],
                    help='dataset: [cifar10|stl10|svhn]')

parser.add_argument('--model', default='resnet', type=str, choices=['resnet'],
...
parser.add_argument('--local_module_num', default=1, type=int,
                    help='number of local modules; paper MAN++ settings are '
                         'ResNet-32 K=8/16 and ResNet-110 K=32/55 '
                         '(1 refers to end-to-end training)')
...
parser.add_argument('--aux_net_config', default='2c2f', type=str,
...
parser.add_argument('--momentum', default=1.0, type=float,) # momentum for EMA

parser.add_argument('--arch', default='resnetInfoPro_MANPP', type=str, choices=sorted(ARCH_ALIASES),
                    help='network module name; *_MAN aliases are accepted for compatibility')
parser.add_argument('--dry-run-model', action='store_true',
                    help='build the model and exit before loading datasets')

args = parser.parse_args()
args.arch = ARCH_ALIASES[args.arch]
...
def validate_args():
    if args.layers not in SUPPORTED_RESNET_LAYERS:
        parser.error(
            f'--layers must be one of {list(SUPPORTED_RESNET_LAYERS)} for this MAN++ release.'
        )

    arch = f'resnet{args.layers}'
    supported_k = supported_local_module_nums(
        arch,
        dataset=args.dataset,
        balanced_memory=args.balanced_memory,
    )
    if args.local_module_num not in supported_k:
        parser.error(
            f'--local_module_num={args.local_module_num} is not configured for {arch} '
            f'on {args.dataset} (balanced_memory={args.balanced_memory}). '
            f'Supported K values: {list(supported_k) if supported_k else "none"}. '
            f'MAN++ paper settings: {_paper_settings_text()}.'
        )


def get_resnet_builder(arch, layers):
    module = importlib.import_module(f'networks.{arch}')
    builder_name = f'resnet{layers}'
    try:
        return getattr(module, builder_name)
    except AttributeError as exc:
        raise ValueError(f'{arch} does not provide {builder_name}().') from exc
...
def load_training_dependencies():
    global np, tqdm, torch, F, cudnn, datasets, transforms
...
experiment_arch = args.arch + ('_balanced' if args.balanced_memory else '')
...
def main():
    ...
    # Create the model before dataset loading so dry-run checks do not download data.
    if args.model == 'resnet':
        resnet_builder = get_resnet_builder(args.arch, args.layers)
    ...
    if args.dry_run_model:
        print(f'Built {args.arch}.resnet{args.layers} with K={args.local_module_num}.')
        return
    ...
    kwargs = {'num_workers': 15, 'pin_memory': True}
