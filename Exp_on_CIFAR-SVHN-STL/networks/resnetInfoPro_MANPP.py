...
from .config import get_infopro_config
...
class MANPPInfoProResNet(nn.Module):
    ...
    def __init__(self, block, layers, arch, local_module_num, batch_size, image_size=32,
        ...
        super(MANPPInfoProResNet, self).__init__()

        if arch not in ['resnet32', 'resnet110']:
            raise ValueError(
                'This MAN++ release supports ResNet-32 and ResNet-110 only. '
                'Use --layers 32 or --layers 110.'
            )
        ...
        self.EMA_first = copy.deepcopy(self.layer1[0])
        self.LB_first = copy.deepcopy(self.layer1[0])
        self.ema_s_first = nn.Parameter(torch.tensor(1.0))


        self.criterion_ce = nn.CrossEntropyLoss()

        self.infopro_config = get_infopro_config(
            arch=arch,
            dataset=dataset,
            local_module_num=local_module_num,
            balanced_memory=balanced_memory,
        )

        exec('self.aux_classifier_' + '0' + '_' + '0' +
                            '= AuxClassifier(wide_list[0],class_num=class_num, '
                            'widen=aux_net_widen, feature_dim=aux_net_feature_dim)')
        exec('self.decoder_' + '0' + '_' + '0' +
                     '= Decoder(wide_list[0], image_size, widen=aux_net_widen)')
        ...
        for param in self.EMA_first.parameters():
            param.requires_grad = False
    ...
    def forward(self, img, target=None,ixx_1=0, ixy_1=0,
        ...
        if self.training:
            ...
            if self.local_module_num == 1:
                ...
                x = self.bn1(x)
                x = self.relu(x)
                y = self.ema_s_first * self.LB_first(x) + (2 - self.ema_s_first) * self.EMA_first(x)
                ratio0 = 0
                ixx_r = ixx_1 * (1 - ratio0) + ixx_2 * ratio0
                ixy_r = ixy_1 * (1 - ratio0) + ixy_2 * ratio0
                loss_ixx = eval('self.decoder_0_0')(y,self._image_restore(img))
                loss_ixy = eval('self.aux_classifier_0_0')(y,target)
                loss = ixx_r * loss_ixx + ixy_r * loss_ixy
                loss.backward()

                x = x.detach()
                for paramEncoder, paramEMA in zip(self.layer1[0].parameters(),self.EMA_first.parameters()):
                    paramEMA.data = paramEMA.data * self.momentum + paramEncoder.data * (1 - self.momentum)

                for i in range(len(self.Encoder_Net) - 1):
                    x = self.Encoder_Net[i](x)
                    y = self.ema_s[i]*self.LB[i](x) + (2-self.ema_s[i])*self.EMA_Net[i](x)
...
def resnet20(**kwargs):
    raise ValueError('Unsupported MAN++ backbone resnet20. Use resnet32 or resnet110.')


def resnet32(**kwargs):
    model = MANPPInfoProResNet(BasicBlock, [5, 5, 5], arch='resnet32', **kwargs)
    return model


def resnet44(**kwargs):
    raise ValueError('Unsupported MAN++ backbone resnet44. Use resnet32 or resnet110.')


def resnet56(**kwargs):
    raise ValueError('Unsupported MAN++ backbone resnet56. Use resnet32 or resnet110.')


def resnet110(**kwargs):
    model = MANPPInfoProResNet(BasicBlock, [18, 18, 18], arch='resnet110', **kwargs)
    return model


def resnet1202(**kwargs):
    raise ValueError('Unsupported MAN++ backbone resnet1202. Use resnet32 or resnet110.')


def resnet164(**kwargs):
    raise ValueError('Unsupported MAN++ backbone resnet164. Use resnet32 or resnet110.')


def resnet1001(**kwargs):
    raise ValueError('Unsupported MAN++ backbone resnet1001. Use resnet32 or resnet110.')


InfoProResNet = MANPPInfoProResNet
