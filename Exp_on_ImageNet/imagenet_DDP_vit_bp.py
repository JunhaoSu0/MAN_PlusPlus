...
from networks.vit_man import PatchEmbed, Block


class BPVisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pdrop(torch.cat((cls, x), 1) + self.pos)
        for blk in self.blocks:
            x = blk(x)
        return self.head(self.norm(x[:, 0]))


def bp_vit_tiny_patch16_224(num_classes=1000, **kw):
    return BPVisionTransformer(embed_dim=192, num_heads=3,
                               num_classes=num_classes, **kw)


def bp_vit_small_patch16_224(num_classes=1000, **kw):
    return BPVisionTransformer(embed_dim=384, num_heads=6,
                               num_classes=num_classes, **kw)


def bp_vit_base_patch16_224(num_classes=1000, **kw):
    return BPVisionTransformer(embed_dim=768, num_heads=12,
                               num_classes=num_classes, **kw)


BP_VIT_FACTORIES = {
    'vit_tiny': bp_vit_tiny_patch16_224,
    'vit_small': bp_vit_small_patch16_224,
    'vit_base': bp_vit_base_patch16_224,
}


parser = argparse.ArgumentParser('ViT-BP DDP')
parser.add_argument('data', metavar='DIR', help='ImageNet root')
parser.add_argument('--net', default='vit_tiny',
                    choices=sorted(BP_VIT_FACTORIES.keys()),
                    help='BP ViT backbone')
...
def main_worker(gpu):
    ...
    create_model = BP_VIT_FACTORIES[args.net]
    if gpu == 0:
        print(f'=> creating {args.net} BP baseline', flush=True)
    model = create_model(num_classes=1000).cuda(gpu)
    if args.distributed:
...
def train(loader, model, criterion, optimizer, epoch, gpu):
    ...
    for i, (images, target) in enumerate(tqdm(loader)):
        ...
        meter_l.update(loss.item(), images.size(0))
...
def validate(loader, model, crit, gpu):
    ...
    for i, (img, tgt) in enumerate(tqdm(loader, desc=f'Val R{gpu}', leave=False)):
        ...
        meter_a1.update(acc1[0], img.size(0))
        meter_a5.update(acc5[0], img.size(0))
        meter_t.update(time.time() - end)
        end = time.time()
        if i % args.print_freq == 0:
            prog.display(i)
